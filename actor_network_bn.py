...
class ActorNetwork:
    ...
    def __init__(self,sess,state_dim,action_dim,ct_dim):
        ...
        self.state_input,self.ct_input,self.action_output,self.net,self.is_training = self.create_network(state_dim,action_dim,ct_dim)

        # create target actor network
        self.target_state_input,self.target_ct_input,self.target_action_output,self.target_update,self.target_is_training = self.create_target_network(state_dim,action_dim,ct_dim,self.net)
    ...
    def create_network(self,state_dim,action_dim,ct_dim):
        ...
        return state_input,ct_input, action_output,[W1,b1,W2,b2,W3,b3],is_training
        '''
        W1 = self.variable([state_dim, layer1_size], state_dim)
        b1 = self.variable([layer1_size], state_dim)
        W2 = self.variable([ct_dim, layer2_size], ct_dim)
        b2 = self.variable([layer2_size], ct_dim)
        W3 = self.variable([layer1_size, layer3_size], layer1_size)
        ...
        layer0_ct_bn = self.batch_norm_layer(ct_input, training_phase=is_training, scope_bn='batch_norm_ct_0',
                                             activation=tf.identity)
        layer1 = tf.matmul(layer0_bn, W1) + b1
        layer1_bn = self.batch_norm_layer(layer1, training_phase=is_training, scope_bn='batch_norm_1',
                                          activation=tf.nn.relu)
        layer2 = tf.matmul(layer0_ct_bn, W2) + b2
        layer2_bn = self.batch_norm_layer(layer2, training_phase=is_training, scope_bn='batch_norm_2',
                                          activation=tf.nn.relu)

        layer3 = tf.matmul(layer1_bn, W3) + tf.matmul(layer2_bn, W3)+ b3
        ...
        return state_input, ct_input, action_output, [W1, b1, W2, b2, W3, b3, W4, b4], is_training


    def create_target_network(self,state_dim,action_dim,ct_dim,net):
        state_input = tf.placeholder("float",[None,state_dim])
        ct_input = tf.placeholder("float", [None, ct_dim])
        is_training = tf.placeholder(tf.bool)
        ...
        return state_input,ct_input, action_output,target_update,is_training
        '''

        layer0_bn = self.batch_norm_layer(state_input, training_phase=is_training, scope_bn='target_batch_norm_0',
                                          activation=tf.identity)
        layer0_ct_bn = self.batch_norm_layer(ct_input, training_phase=is_training, scope_bn='target_batch_norm_ct_0',
                                          activation=tf.identity)

        layer1 = tf.matmul(layer0_bn, target_net[0]) + target_net[1]
        layer1_bn = self.batch_norm_layer(layer1, training_phase=is_training, scope_bn='target_batch_norm_1',
                                          activation=tf.nn.relu)
        layer2 = tf.matmul(layer0_ct_bn, target_net[2]) + target_net[3]
        layer2_bn = self.batch_norm_layer(layer2, training_phase=is_training, scope_bn='target_batch_norm_2',
                                          activation=tf.nn.relu)
        layer3 = tf.matmul(layer1_bn, target_net[4]) + tf.matmul(layer2_bn, target_net[4]) + target_net[5]
    ...
    def train(self,q_gradient_batch,state_batch,ct_batch):
    ...
    def actions(self,state_batch,ct_batch):
    ...
    def target_actions(self,state_batch,ct_batch):
    ...
    def save_network(self,time_step):
        ...
        self.saver = tf.train.Saver()
        self.saver.save(self.sess, 'saved_actor_networks_withoutc/' + 'actor-network', global_step = time_step)
